...
export async function getGoogleCalendarEvents(accessToken) {
  ...
  // 取得期間を前後30日に設定（翌日以降も見えるようにするため）
  const now = new Date();
  const timeMinDate = new Date(now);
  timeMinDate.setDate(timeMinDate.getDate() - 30);
  ...
  try {
    ...
    const allEventsPromises = activeCalendars.map(async (cal) => {
      // 特定のカレンダーを除外 (廣瀬さん、祝日など)
      const isHirose = cal.id === 'hirose@signstar.network' || cal.summary?.includes('廣瀬');
      const isHoliday = cal.id?.includes('holiday@group.v.calendar.google.com') || cal.summary?.includes('祝日') || cal.summary?.includes('Holiday');
      
      if (isHirose || isHoliday) {
        console.log(`[CALENDAR] Skipping calendar: ${cal.summary} (${cal.id})`);
        return [];
      }
